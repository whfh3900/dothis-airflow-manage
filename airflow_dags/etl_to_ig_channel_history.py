...
def run_etl():
    command = ['/app/miniconda3/condabin/conda', 'run', '-n', 'igniteClient', '--no-capture-output', '/app/miniconda3/envs/igniteClient/bin/python', '/app/airflow/dags/to_ignite_etl/etl-one-day-channel_history.py']
    try:
...
# DAG 정의
dag = DAG(
    'etl_to_ig_channel_history',
    default_args = args,
    description='channel_history ETL from mariadb to ignite',
