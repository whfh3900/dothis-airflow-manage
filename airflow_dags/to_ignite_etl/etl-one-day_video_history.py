...
yyyymm_str=now.strftime('%Y%m') # 2024년 4월의 경우 202404 로 선언됨
today_str=now.strftime('%Y%m%d') # 2024년 4월 18일의 경우 20240418로 선언됨
...
#for i in range(1):
    number_str = f"{i:02}"  # 2자리 숫자로 포맷팅
#    number_str = 85
    table_mapping[f'dothis_exp.video_history_{number_str}_' + yyyymm_str] = {
        'target_table': f'dothis.video_history_{number_str}_' + yyyymm_str,
        'columns': ['video_id','video_views','video_likes','video_comments','video_performance','year','month','day','video_cluster'],
        'where_clause': 'year*10000+month*100+day in (' +  today_str + ',' + one_day_before_str + ',' + two_days_before_str + ')'
    }

# 예외적인 숫자들 추가
exceptional_numbers = ['100', 'NULL']  # 9999는 제외
for number in exceptional_numbers:
    table_mapping[f'dothis_exp.video_history_{number}_' + yyyymm_str] = {
        'target_table': f'dothis.video_history_{number}_' + yyyymm_str,
        'columns': ['video_id','video_views','video_likes','video_comments','video_performance','year','month','day','video_cluster'],
        'where_clause': 'year*10000+month*100+day in (' +  today_str + ',' + one_day_before_str + ',' + two_days_before_str + ')'
    }
...
def transfer_data(mysql_cursor, ignite_client, source_table, mapping_info):
    ...
    for row in cleaned_rows:
        try:
            ...
            query_str = f"/* SQL00104 */MERGE INTO {target_table} ({', '.join(columns)}) VALUES ({values_str})"
    ...
    # Ignite에서 데이터 건수 확인
    # Ignite 쿼리 실행 방식에 따라 적절한 방법으로 수정해야 합니다.
