...
print('yyyymm_str:', yyyymm_str)
print('one_day_before_str:'+one_day_before_str)

# 테이블 매핑 정보: 초기적재는 전체 건수를 가져와야 하므로 where 절에 1=1으로 처리
table_mapping = {
    'dothis_exp.channel_history_'+yyyymm_str: {'target_table': 'dothis.channel_history_'+yyyymm_str, 'columns': ['channel_id','channel_average_views','channel_subscribers','channel_total_views','channel_total_videos','year','month','day'],
...
def transfer_data(mysql_cursor, ignite_client, source_table, mapping_info):
    ...
    for row in cleaned_rows:
        try:
            ...
            query_str = f"/* SQL00102 */MERGE INTO {target_table} ({', '.join(columns)}) VALUES ({values_str})"
