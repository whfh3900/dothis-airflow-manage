...
for i in range(100):
    ...
    table_mapping[f'dothis_exp.video_data_{number_str}'] = {
        'target_table': f'dothis.video_data_{number_str}',
        'columns': ['video_id', 'channel_id', 'video_title', 'video_description', 'video_tags', 'video_duration', 'video_published', 'video_category', 'video_info_card', 'video_with_ads', 'video_end_screen', 'video_cluster', 'crawled_date', 'year', 'month', 'day'],
        'where_clause': 'year*10000+month*100+day in (' + today_str +','+one_day_before_str+')'
    }

# 예외적인 숫자들 추가
exceptional_numbers = ['9999', '100', 'NULL']
for number in exceptional_numbers:
    table_mapping[f'dothis_exp.video_data_{number}'] = {
        'target_table': f'dothis.video_data_{number}',
        'columns': ['video_id', 'channel_id', 'video_title', 'video_description', 'video_tags', 'video_duration', 'video_published', 'video_category', 'video_info_card', 'video_with_ads', 'video_end_screen', 'video_cluster', 'crawled_date', 'year', 'month', 'day'],
        'where_clause': 'year*10000+month*100+day in (' + today_str +','+one_day_before_str+')' 
    }
...
def transfer_data(mysql_cursor, ignite_client, source_table, mapping_info):
    ...
    for row in cleaned_rows:
        try:
            ...
            query_str = f"/* SQL00103 */MERGE INTO {target_table} ({', '.join(columns)}) VALUES ({values_str})"
