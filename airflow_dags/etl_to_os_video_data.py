...
# DAG 정의
dag = DAG(
    'etl_to_os_video_data',
    default_args=args,
    description="video_data 클러스터링 후 OpenSearch에 저장",
...
bash_command = f"/app/logstash/bin/logstash -f /app/logstash/config/etl_to_opensearch/video_data/video_data_{today}.conf"
ssh_domain = "svcmon@es-sa"

# BashOperator를 사용하여 명령어 실행
run_etl_task = BashOperator(
    task_id='run_os_vd_etl',
