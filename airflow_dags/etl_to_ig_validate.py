...
def run_etl():
    command = ['/app/miniconda3/condabin/conda', 'run', '-n', 'igniteClient', '--no-capture-output', '/app/miniconda3/envs/igniteClient/bin/python', '/app/airflow/dags/to_ignite_etl/validate_cnt.py']
    try:
...
dag = DAG(
    'etl_to_ig_validate',
    default_args = args,
    description='validate count mariadb(src) with ignite(dst)',
