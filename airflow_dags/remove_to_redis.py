...
from redis_update_util.redis_remove import remove_old_data
...
with open('/app/airflow/dags/config.json', 'r') as f:
    default_args = json.load(f)['remove_to_redis']
...
remove_to_redis_task = PythonOperator(
    task_id='remove_to_redis_task',
    python_callable=remove_old_data,  # 실행될 함수
    provide_context=True,
    queue="ai",
...
schedule_start_task >> remove_to_redis_task
