...
def run_etl():
    command = ['/app/miniconda3/condabin/conda', 'run', '-n', 'igniteClient', '--no-capture-output', '/app/miniconda3/envs/igniteClient/bin/python', '/app/airflow/dags/to_ignite_etl/etl-one-day_video_history_shorts.py']
    try:
...
# DAG 정의
dag = DAG(
    'etl_to_ig_video_history_shorts',
    default_args = args,
    description='video_history_shorts  ETL from mariadb to ignite ',
