...
sys.path.append('./')
load_dotenv()
CONSUME_TOPIC = "video-history-shorts-crawled"
GROUP_ID = "video-history-shorts-mysql-consumer"
NUM_PARTITION = 5
...
async def history_raw_load():
    ...
    insert_query = f"""
        INSERT IGNORE INTO dothis_raw.video_history_shorts_{today_str.replace("-", "")}
        ({", ".join(cols)})
