...
async def update_need_crawling():
    ...
    async def process_msg_batch(msg_list):
        ...
        for msg in record_list:
            if msg.value.get("msg"):
                ...
                value = msg.value
                query_list.append(f"UPDATE request_video SET is_need_crawling = 0 WHERE video_id = '{value['video_id']}'")  
        if query_list != []:
    ...
    try:
        while True:
            ...
            if all(partition_eof.values()) or msg_list == {}:
                break
        print(f"new video count : {video_count}")
    finally:
        await consumer.stop()
        conn.close()


def rds_update_need_crawling():
    asyncio.run(update_need_crawling())