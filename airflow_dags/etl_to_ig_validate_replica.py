...
def run_etl():
    command = ['/app/miniconda3/condabin/conda', 'run', '-n', 'igniteClient', '--no-capture-output', '/app/miniconda3/envs/igniteClient/bin/python', '/app/airflow/dags/to_ignite_etl/validate_replica.py']
    try:
...
# DAG 정의
dag = DAG(
    'etl_to_ig_validate_replica',
    default_args = args,
    description='mariadb master slave replica validation',
